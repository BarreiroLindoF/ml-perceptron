...
class perceptron(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################################
        scores = np.sign(X.dot(self.params['W']))
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        #########################################################################
        # Get indeces of badly classified instances
        bad_indeces = np.not_equal(scores, y)

        # Filter by indeces. Ravel() is used so that a boolean value is used for every element of specific row
        bad_predictions_X = X[bad_indeces.ravel()]
        bad_predictions_y = y[bad_indeces]

        loss = -np.sum(bad_predictions_X.dot(self.params['W'])*bad_predictions_y)
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
        ...
        #########################################################################
        grads['W'] = (X.T.dot(y - np.sign(X.dot(self.params['W']))))
    ...
    def train(self, X, y, learning_rate=1e-3, num_epochs=1000, batch_size=10, display=False):
        ...
        #########################################################################
        training_data = X.reshape((-1, batch_size, X.shape[1]))
        training_labels = y.reshape((-1, batch_size))
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
    ...
    def predict(self, X):
        ...
        ###########################################################################
        y_pred = np.sign(X.dot(self.params['W']))
        ###########################################################################
        #                              END OF YOUR CODE                           #
        ###########################################################################
